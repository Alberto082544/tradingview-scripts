"""
Ranger C AUDCAD — Mean Reversion H4+M15
Familia: Ranger C | Par: AUDCAD (ambas direcciones)

AUDCAD: dos divisas commodity correladas (AUD=hierro/China, CAD=petroleo).
Precio historico 0.85-1.05, sin tendencias sostenidas largas.

  pip = 0.0001 (4 decimales)
  pip_val = pip * 100000 * CADUSD  (cotizacion en CAD -> USD)
  Sin filtro EMA — AUDCAD es de rango por naturaleza
  ADX H4 < umbral como unico filtro de regimen

H4: ADX < ADX_H4_Max (mercado lateral)
...
PIP    = 0.0001
CADUSD = 0.73   # aproximacion: 1/USDCAD ~ 1/1.37
...
def run_backtest(df: pd.DataFrame, params: dict = None,
                 ...
                 cadusd: float = CADUSD) -> pd.DataFrame:
    p       = DEFAULT_PARAMS if params is None else params
