"""
Ranger C AUDNZD — Mean Reversion H4+M15
Familia: Ranger C | Par: AUDNZD (ambas direcciones)

Mismo concepto que ranger_c_mrv_m15.py pero adaptado para AUDNZD:
  - pip = 0.0001 (4 decimales)
  - pip_val = pip * 100000 * NZDUSD (en USD por pip por lote)
  - Sin filtro de tendencia H4 — AUDNZD es un par de rango por naturaleza
  - ADX H4 < umbral como unico filtro de regimen

H4: ADX < ADX_H4_Max (mercado lateral, optimo para mean reversion)
M15: precio toca banda BB + RSI en zona + RSI girando

AUDNZD | pip = 0.0001 | NZDUSD ~ 0.60
"""
import pandas as pd
import numpy as np

PIP    = 0.0001
NZDUSD = 0.60   # aproximacion conservadora
...
DEFAULT_PARAMS = {
    ...
    'TrailDistPips':    10,
    'ExitBars':         16,
    'SessionStart':      0,   # AUDNZD activo sesion asiatica
    'SessionEnd':       23,
    'BadHour':          -1,   # sin hora mala por defecto
    'MaxTradesDay':      5,
    'LotRiskPct':      0.7,
    'MaxLots':         4.0,
}
...
def run_backtest(df: pd.DataFrame, params: dict = None,
    ...
    for i in range(warmup, len(df)):
        ...
        if pos is not None:
            ...
            if pos['dir'] == 'long':
                if l_i <= pos['sl']:
                    ...
                    ep, et = pos['tp'], 'TP'
                elif bb_mid_tp and h_i >= pos['bb_mid_entry']:
                    ep, et = pos['bb_mid_entry'], 'BB_MID'
            else:
                if h_i >= pos['sl']:
                    ep, et = pos['sl'], 'SL'
                elif l_i <= pos['tp']:
                    ep, et = pos['tp'], 'TP'
                elif bb_mid_tp and l_i <= pos['bb_mid_entry']:
                    ep, et = pos['bb_mid_entry'], 'BB_MID'
            ...
            if ep is not None:
                if pos['dir'] == 'long':
                    ...
                    pnl_usd = (pos['entry'] - ep) / PIP * pip_val * pos['lots']
                equity += pnl_usd
        ...
        hr = dt.hour
        if hr < sess_s or hr >= sess_e:
            continue
        if hr == bad_h:
            continue
        if dt.weekday() >= 5 or (dt.weekday() == 4 and hr >= 22):
            continue
        if trades_today >= max_td:
            continue

        adx1 = adx_arr[i-1]
        if np.isnan(adx1) or adx1 >= adx_max:
            continue

        atr1 = atr_arr[i-1]
        rsi1 = rsi_arr[i-1]; rsi2 = rsi_arr[i-2]
        if np.isnan(atr1) or atr1 <= 0 or np.isnan(rsi1):
            continue

        cond_long  = (c[i-1] <= bb_lo[i-1] * 1.001 and
                      rsi1 < rsi_lo and
                      (not rsi_confirm or rsi1 > rsi2))
        cond_short = (not cond_long and
                      c[i-1] >= bb_up[i-1] * 0.999 and
                      rsi1 > rsi_hi and
                      (not rsi_confirm or rsi1 < rsi2))
        ...
        entry = o_i
        risk  = equity * lot_risk
        lots  = min(round(risk / (sl_dist / PIP * pip_val), 2), max_lots)
        lots  = max(lots, 0.01)
