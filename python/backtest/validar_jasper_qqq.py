"""Validacion completa Jasper OB combo C en QQQ: year-by-year + MC + checklist 7"""
import sys, os
...
# Combo C ganador: EMA200 H4 + Wick
PARAMS = {**DEFAULT_PARAMS, 'UseEMA200H4':1, 'UseWick':1}
...
def main():
    print("="*78)
    print("  VALIDACION  Jasper OB + EMA200 H4 + Wick  en QQQ (NAS100)")
    print("="*78)
    ...
    print(f"\n  MONTE CARLO OOS ({N_SIM} sims):")
    print(f"    DD real OOS: {m_oos['dd_pct']}%")
    ...
    print(f"  Profit OOS: PF={m_oos['pf']} DD={m_oos['dd_pct']}% Ann={m_oos['ann_pct']}% N={m_oos['n']}")
    print(f"  Comparado con EMA9+VWAP+RSI QQQ (PF=1.76 DD=3.0% Ann=17.1% N=165):")
    print(f"    Jasper OB es {'PEOR' if m_oos['pf']<1.76 else 'MEJOR'} en PF")
