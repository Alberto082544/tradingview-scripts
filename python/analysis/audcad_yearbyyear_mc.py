"""
Validacion regimen-dependencia para el top combo AUDCAD con Estocastico.

- Year-by-year: PF, PnL, DD para cada año 2014-2025
...
# Top combo de la optimizacion del 2026-05-18
TOP_PARAMS = {
    # Indicadores
    "BB_Period": 20,
    "BB_StdDev": 2.0,
    ...
    "RSI_Short_Min": 60,
    "RSI_Confirm": 1,
    "StochMode": 2,            # ambos (RSI + Stoch) confirman
    "Stoch_Long_Max": 20,
    "Stoch_Short_Min": 75,
    "BB_Mid_TP": 0,
    "MinSLPips": 20,
...
def calc_dd_pct(pnl_series, initial=CAPITAL):
    """DD maximo en %, asumiendo equity = initial + cumsum(pnls)."""
...
def main():
    print("=" * 70)
    print("  AUDCAD Top Combo - Validacion Year-by-Year + Monte Carlo")
    print("=" * 70)

    print("  Cargando datos AUDCAD M15...")
    ...
    consistency_ok = positive_years >= (total_years * 10 / 12)  # >=10/12 proporcional
    ...
    for i in range(MC_RUNS):
        ...
        if pnl_total > 0:  # invariante en permutacion
            pct_pos_sims += 1
    ...
    # ---- Guardar ----
    out_yearly = os.path.join(OUT_DIR, "AUDCAD_top_yearly.csv")
    pd.DataFrame(yearly_rows).to_csv(out_yearly, index=False)
    out_mc = os.path.join(OUT_DIR, "AUDCAD_top_mc_dd_distribution.csv")
    ...
    out_json = os.path.join(OUT_DIR, "AUDCAD_top_validation_summary.json")
